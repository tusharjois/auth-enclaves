...
int ecall_generate_key(unsigned char* in_key, unsigned char* out_key)
{
  if (NULL == hmac_key) {
  ...
  if (NULL == in_key) {
    ...
    sgx_status_t ret = sgx_read_rand(hmac_key, USM_HMAC_KEY_M);
...
int ecall_authenticate_outgoing_msg(const unsigned char *whole_msg, size_t msg_len, unsigned char *authenticated_whole_msg)
{
  if (NULL == hmac_key) {
  ...
  // sgx_status_t ret = sgx_hmac_sha256_msg(authenticated_whole_msg, msg_len, hmac_key, USM_HMAC_KEY_M, auth_params, USM_HMAC_N);
...
int ecall_authenticate_incoming_msg(const unsigned char *whole_msg, size_t msg_len, unsigned char *authenticated_whole_msg)
{
  if (NULL == hmac_key) {
    return STATUS_ERR_NKEY;
  }
